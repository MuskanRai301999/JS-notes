...
const contacts = [
  ...
  function mapContactsToCompanies(contacts) {
      // Use the reduce function to group contacts by company
  const groupedContacts = contacts.reduce((result, contact) => {
    const { name, company } = contact;

    // If the company doesn't exist in the result object, create an empty array for it
    if (!result[company]) {
      result[company] = [];
    }

    // Push the contact name to the company's array
